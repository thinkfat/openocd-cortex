...
static int dap_setup_accessport_csw(struct adiv5_dap *dap, uint32_t csw)
{
	csw = csw | CSW_DBGSWENABLE | CSW_MASTER_DEBUG | CSW_HPROT |
		dap->ap[dap->ap_current >> 24].csw_default;

	if (csw != dap->ap[dap->ap_current >> 24].csw_value) {
		/* LOG_DEBUG("DAP: Set CSW %x",csw); */
		int retval = dap_queue_ap_write(dap, AP_REG_CSW, csw);
		if (retval != ERROR_OK)
			return retval;
		dap->ap[dap->ap_current >> 24].csw_value = csw;
	}
	return ERROR_OK;
}
...
{
	if (tar != dap->ap[dap->ap_current >> 24].tar_value ||
			(dap->ap[dap->ap_current >> 24].csw_value & CSW_ADDRINC_MASK)) {
		/* LOG_DEBUG("DAP: Set TAR %x",tar); */
		int retval = dap_queue_ap_write(dap, AP_REG_TAR, tar);
		if (retval != ERROR_OK)
			return retval;
		dap->ap[dap->ap_current >> 24].tar_value = tar;
	}
	return ERROR_OK;
}
...
{
	struct adiv5_ap *ap = &dap->ap[dap->ap_current >> 24];
